...
export default function GameStats({ moves, time, matched, total }: GameStatsProps) {
  return (
    <div className="flex items-center justify-center gap-6 flex-wrap">
      <div className="flex flex-col items-center bg-slate-800 rounded-xl px-6 py-3 border border-slate-700 min-w-[90px]">
        <span className="text-slate-400 text-xs font-medium uppercase tracking-wide">Moves</span>
        <span className="text-2xl font-bold text-white mt-1">{moves}</span>
      </div>
      <div className="flex flex-col items-center bg-slate-800 rounded-xl px-6 py-3 border border-slate-700 min-w-[90px]">
        <span className="text-slate-400 text-xs font-medium uppercase tracking-wide">Time</span>
        <span className="text-2xl font-bold text-indigo-400 mt-1">{formatTime(time)}</span>
      </div>
      <div className="flex flex-col items-center bg-slate-800 rounded-xl px-6 py-3 border border-slate-700 min-w-[90px]">
        <span className="text-slate-400 text-xs font-medium uppercase tracking-wide">Pairs</span>
        <span className="text-2xl font-bold text-emerald-400 mt-1">{matched}/{total}</span>
      </div>
    </div>
  );
}
